...
function renderReviews(reviews) {
  ...
  reviews.forEach(review => {
    ...
    const isAuthor = currentUserId.length > 0 && reviewAuthorId
        === currentUserId;

    // 💡 디버깅 로그: 이 로그를 꼭 확인해 주세요!
    console.log(
        `Review ID: ${review.id}, Author ID: [${reviewAuthorId}], Current User ID: [${currentUserId}], Is Author: ${isAuthor}`);
    ...
    console.log("reviews", review.profImg)
...
function renderPagination(pagination) {
   ...
   */
  const createPageLinkHtml = (label, targetPage, isDisabled, isActive,
      actionType = 'page') => {
    const activeClass = isActive ? ' active' : '';
    const disabledStyle = isDisabled
        ? ' style="opacity: 0.5; pointer-events: none;"' : '';
    const targetAttr = `data-${actionType === 'page' ? 'page'
  ...
  const prevGroupHtml = createPageLinkHtml('‹', prevGroupPage,
      isPrevGroupDisabled, false, 'prev-group');
  ...
  const nextGroupHtml = createPageLinkHtml('›', nextGroupPage,
      isNextGroupDisabled, false, 'next-group');
  paginationContainer.insertAdjacentHTML('beforeend', nextGroupHtml);

  // -------------------------------------------------------------
  // 5. [마지막 »] 버튼: 항상 totalPages로 이동
  // -------------------------------------------------------------
  const lastPageHtml = createPageLinkHtml('»', totalPages,
